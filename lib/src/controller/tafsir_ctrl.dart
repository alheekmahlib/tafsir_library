...
class TafsirCtrl extends GetxController {
      ...
      if (selectedTafsir.type == TafsirFileType.json) {
        ...
        if (selectedTafsir.fileName == _defaultDownloadedTafsirName) {
          jsonString = await _gzipJsonService.loadText(
            'packages/tafsir_library/assets/$_defaultDownloadedTafsirName.json.gz',
          );
        } else {
          if (kIsWeb) {
            final url =
  ...
  Future<List<TafsirTableData>> fetchTafsirAyah(int ayahUQNumber,
      {String? databaseName}) async {
    return tafseerList.where((e) => e.id == ayahUQNumber).toList();
  }
  ...
  Future<void> fetchTranslate() async {
    try {
      ...
      if (radioValue.value == translationsStartIndex) {
        jsonString = await _gzipJsonService.loadText(
          'packages/tafsir_library/assets/en.json.gz',
          fallbackPlainAssetPath: 'packages/tafsir_library/assets/en.json',
        );
      } else if (kIsWeb) {
        final url =
  ...
  TranslationModel? getTranslationForAyahModel(
      int surahNumber, int ayahNumber, int ayahIndex) {
    if (kDebugMode) {
      print(
          'Looking for translation: Surah $surahNumber, Ayah $ayahNumber, Index $ayahIndex, Total: ${translationList.length}');
    }
...
class CustomTafsirEntry {
  ...
  int ayahUnqNum(int ayahNum) => ayahUQNumber;
  ...
  factory CustomTafsirEntry.fromJson(Map<String, dynamic> j) =>
      CustomTafsirEntry(
        ...
        ayahUQNumber: j['ayahUQNumber'] as int,
      );
